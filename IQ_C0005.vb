...
Public Class IQ_C0005
    ...
    Private Sub Button7_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button7.Click
        Dim NuevoTicket As New PictureBox()
        PanelDesign.Controls.Add(NuevoTicket)
        NuevoTicket.Height = 300
        NuevoTicket.Width = 300
        NuevoTicket.BorderStyle = BorderStyle.Fixed3D
        NuevoTicket.Image = System.Drawing.Image.FromFile(Disco_Appl & ":\I-Q\Iconos\Ticket2.Png")
        NuevoTicket.SizeMode = PictureBoxSizeMode.StretchImage
        NuevoTicket.Location = New System.Drawing.Point(0, 0)
        controles(num_controles, 0) = Busca_Maximo("Ticket")
        controles(num_controles, 1) = "Ticket"
        controles(num_controles, 2) = ""
        controles(num_controles, 3) = ""
        controles(num_controles, 5) = CStr(num_controles)
        NuevoTicket.Name = controles(num_controles, 0)
        num_controles += 1
        NuevoTicket.BringToFront()
        Dim tooltip1 As New ToolTip(Me.components)
        tooltip1.SetToolTip(NuevoTicket, "Area de Despliegue de Tickets")
        AddHandler NuevoTicket.Click, AddressOf Control_Click
        AddHandler NuevoTicket.MouseDown, AddressOf Control_MouseDown
        AddHandler NuevoTicket.MouseMove, AddressOf Control_MouseMove
        AddHandler NuevoTicket.MouseUp, AddressOf Control_MouseUp
        Control_a_cargo = NuevoTicket
        Nro_Control = num_controles - 1
        Me.txtNombre.Text = Control_a_cargo.name
        Me.txtTipo.Text = controles(num_controles - 1, 1)
        Me.txtAlto.Text = CStr(Control_a_cargo.Height)
    ...
    Private Sub Limpia_Oficinas(Codigo As String)
        Try
            ...
            IQ_Cnn.Open()
            Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from Iq_Design Where IQDesign_Oficina = '" + Codigo & "' And IQDesign_Area = '' And IQDesign_Punto = '' And IQDesign_Tipo = 'L'", IQ_Cnn)
    ...
    Private Sub LImpia_Areas(Codigo As String)
        ...
        While Carga_Reader_O.Read
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from Iq_Design Where IQDesign_Oficina = '' And IQDesign_Area = 'A:" & Trim(Carga_Reader_O.GetValue(0)) & "' And IQDesign_Punto = '' And IQDesign_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
            Catch exc As Exception
                Dim Mensaje_Excepcion As String
    ...
    Private Sub LImpia_Puntos(Codigo As String)
        ...
        While Carga_Reader_O.Read
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from Iq_Design Where IQDesign_Oficina = '' And IQDesign_Area = '' And IQDesign_Punto = '" & Trim(Carga_Reader_O.GetValue(0)) & "' And IQDesign_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
            Catch exc As Exception
                Dim Mensaje_Excepcion As String
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        toolTip1.SetToolTip(CmdRefresh, "Actualizar_Listas")
        ...
        toolTip1.SetToolTip(Me.Button7, "Despliegue de Tickets")
    ...
    Private Sub LImpia_LCD(Codigo As String)
        ...
        While Carga_Reader_O.Read
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from Iq_Design Where IQDesign_Oficina = '' And IQDesign_Area = 'L:" & Trim(Carga_Reader_O.GetValue(0)) & "' And IQDesign_Punto = '' And IQDesign_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
            Catch exc As Exception
                Dim Mensaje_Excepcion As String
    ...
    Private Sub CmdExec_Click(sender As Object, e As EventArgs) Handles CmdExec.Click
        ...
        Try
            Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
            IQ_Cnn.Open()
            Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from Iq_Design Where IQDesign_Oficina = '" + alcance(0) & "' And IQDesign_Area = '" + alcance(1) & "' And IQDesign_Punto = '" + alcance(2) & "' And IQDesign_Tipo = 'L'", IQ_Cnn)
            Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
            IQ_Cnn.Close()
        Catch exc As Exception
            Dim Mensaje_Excepcion As String
                ...
                While Carga_Reader_O2.Read
                    If IsDBNull(Carga_Reader_O2.GetValue(0)) = False Then
                        Try
                            Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                            IQ_Cnn.Open()
                            Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from Iq_Design Where IQDesign_Oficina = '' And IQDesign_Area = '' And IQDesign_Punto = '" + Carga_Reader_O2.GetValue(0) & "' And IQDesign_Tipo = 'L'", IQ_Cnn)
                            Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                            IQ_Cnn.Close()
                        Catch exc As Exception
                            Dim Mensaje_Excepcion As String
        ...
        If alcance(0) = "999999" Then
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from Iq_Design Where IQDesign_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
                GoTo fin_limpia
            Catch exc As Exception
...
Fin_Limpia:
        ...
        For counter = 0 To num_controles
            If Trim(controles(counter, 0)) <> "" Then
                ...
                instruccion_insert = "Insert Into Iq_Design Values('" + alcance(0) + "', '" + alcance(1) + "', '" & alcance(2) + "', 'L', "
    ...
    Private Sub CmdReport_Click(sender As Object, e As EventArgs) Handles CmdReport.Click
        ...
        Try
            rptlayout.Load(Disco_Appl & ":\I-Q\IQ_Rpt\IQ_C0005.rpt")
            rptlayout.SetDatabaseLogon(Server_User, Server_Pwd)
            rptlayout.SummaryInfo.ReportTitle = Nombre_Cliente + Chr(10) + Me.Text
            Me.crvReporte.SelectionFormula = "{IQ_Design.IQDesign_Oficina} = '" + alcance(0) + "' And {IQ_Design.IQDesign_Area} = '" + alcance(1) + "' And {IQ_Design.IQDesign_Punto} = '" + alcance(2) + "' And {IQ_Design.IQDesign_Tipo} = 'L'"
    ...
    Private Sub CmdSearch_Click(sender As Object, e As EventArgs) Handles CmdSearch.Click
        ...
        Dim Carga_Comando_O1 As New OleDb.OleDbCommand("Select IQ_Design.*, IQ_PuntosAtencion.IQPUntos_Descripcion from IQ_Design join IQ_PuntosAtencion on IQ_Design.IIQDesign_Punto = IQ_PuntosAtencion.IQPuntos_Codigo where IQDesign_Oficina = '' and IQDesign_Area = '' and IQDEsign_Punto = '" & Alcance_recup(2) & "' And IQDesign_Tipo = 'L' Order by IQDesign_Orden", Carga_Coneccion_O1)
