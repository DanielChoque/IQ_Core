...
Public Class IQ_C0007
    ...
    Private Sub Limpia_Oficinas(Codigo As String)
        Try
            ...
            IQ_Cnn.Open()
            Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from IQ_MMConfig Where IQMMConfig_Oficina = '" + Codigo & "' And IQMMConfig_Area = '' And IQMMConfig_Punto = '' And IQMMConfig_Tipo = 'L'", IQ_Cnn)
        ...
        While Carga_Reader_O.Read
            ...
            LImpia_LCD(Carga_Reader_O.GetValue(0))
        End While
        Carga_Coneccion_O.Dispose()
    End Sub
    Private Sub LImpia_Areas(Codigo As String)
        ...
        While Carga_Reader_O.Read
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from IQ_MMConfig Where IQMMConfig_Oficina = '' And IQMMConfig_Area = 'A:" & Trim(Carga_Reader_O.GetValue(0)) & "' And IQMMConfig_Punto = '' And IQMMConfig_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
            Catch exc As Exception
                Dim Mensaje_Excepcion As String
    ...
    Private Sub LImpia_Puntos(Codigo As String)
        ...
        While Carga_Reader_O.Read
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from IQ_MMConfig Where IQMMConfig_Oficina = '' And IQMMConfig_Area = '' And IQMMConfig_Punto = '" & Trim(Carga_Reader_O.GetValue(0)) & "' And IQMMConfig_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
            Catch exc As Exception
                Dim Mensaje_Excepcion As String
    ...
    Private Sub TrvOficinas_KeyDown(sender As Object, e As KeyEventArgs) Handles TrvOficinas.KeyDown
        If e.Control AndAlso e.KeyCode = Keys.C Then
            ...
            End If
            Try
                If MessageBox.Show("Está Ud. Seguro de definir " & labelito & " Como la Configuraciòn Multimedia a recuperar?", Me.Text, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Cancel Then
                    alcance_control_c(0) = ""
                    alcance_control_c(1) = ""
                    alcance_control_c(2) = ""
                    Exit Sub
                ...
                Else
                    ...
                    End If
                End If
                MessageBox.Show(labelito & " ESTABLECIDO COMO CONFIGURACIÒN MULTIMEDIA A RECUPERAR", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
            Catch ex As Exception
                Exit Sub
            End Try
        End If
    End Sub
    Private Sub LImpia_LCD(Codigo As String)
        Dim Carga_Coneccion_O As New OleDb.OleDbConnection(Cnn_Central_Server)
        Carga_Coneccion_O.Open()
        Dim Carga_Comando_O As New OleDb.OleDbCommand("Select IqConfigOfic_Codigo from IQ_ConfigOfic where IqConfigOfic_Oficina = '" & Codigo & "' And IqConfigOfic_Tipo = 'L'", Carga_Coneccion_O)
        Dim Carga_Reader_O As OleDb.OleDbDataReader = Carga_Comando_O.ExecuteReader(CommandBehavior.CloseConnection)
        While Carga_Reader_O.Read
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from IQ_MMConfig Where IQMMConfig_Oficina = '' And IQMMConfig_Area = 'L:" & Trim(Carga_Reader_O.GetValue(0)) & "' And IQMMConfig_Punto = '' And IQMMConfig_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
            Catch exc As Exception
                Dim Mensaje_Excepcion As String
    ...
    Private Sub CmdExec_Click(sender As Object, e As EventArgs) Handles CmdExec.Click
        ...
        Try
            Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
            IQ_Cnn.Open()
            Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from IQ_MMConfig Where IQMMConfig_Oficina = '" + alcance(0) & "' And IQMMConfig_Area = '" + alcance(1) & "' And IQMMConfig_Punto = '" + alcance(2) & "' And IQMMConfig_Tipo = 'L'", IQ_Cnn)
            Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
            IQ_Cnn.Close()
        Catch exc As Exception
            Dim Mensaje_Excepcion As String
                ...
                While Carga_Reader_O2.Read
                    If IsDBNull(Carga_Reader_O2.GetValue(0)) = False Then
                        Try
                            Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                            IQ_Cnn.Open()
                            Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from IQ_MMConfig Where IQMMConfig_Oficina = '' And IQMMConfig_Area = '' And IQMMConfig_Punto = '" + Carga_Reader_O2.GetValue(0) & "' And IQMMConfig_Tipo = 'L'", IQ_Cnn)
                            Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                            IQ_Cnn.Close()
                        Catch exc As Exception
                            Dim Mensaje_Excepcion As String
        ...
        If alcance(0) = "999999" Then
            Try
                Dim IQ_Cnn As New OleDb.OleDbConnection(Cnn_Central_Server)
                IQ_Cnn.Open()
                Dim IQ_Cmm As New OleDb.OleDbCommand("Delete from IQ_MMConfig Where IQMMConfig_Tipo = 'L'", IQ_Cnn)
                Dim RegistrosEliminados As Long = IQ_Cmm.ExecuteNonQuery()
                IQ_Cnn.Close()
                GoTo fin_limpia
            Catch exc As Exception
...
Fin_Limpia:
        ...
        For counter = 0 To num_controles
            If Trim(controles(counter, 0)) <> "" Then
                ...
                instruccion_insert = "Insert Into IQ_MMConfig Values('" + alcance(0) + "', '" + alcance(1) + "', '" & alcance(2) + "', 'L', "
    ...
    Private Sub CmdReport_Click(sender As Object, e As EventArgs) Handles CmdReport.Click
        ...
        Try
            rptlayout.Load(Disco_Appl & ":\I-Q\IQ_Rpt\IQ_C0007.rpt")
            rptlayout.SetDatabaseLogon(Server_User, Server_Pwd)
            rptlayout.SummaryInfo.ReportTitle = Nombre_Cliente + Chr(10) + Me.Text
            Me.crvReporte.SelectionFormula = "{IQ_MMConfig.IQMMCOnfig_Oficina} = '" + alcance(0) + "' And {IQ_MMCOnfig.IQMMCOnfig_Area} = '" + alcance(1) + "' And {IQ_MMCOnfig.IQMMCOnfig_Punto} = '" + alcance(2) + "' And {IQ_MMCOnfig.IQMMCOnfig_Tipo} = 'L'"
    ...
    Private Sub CmdSearch_Click(sender As Object, e As EventArgs) Handles CmdSearch.Click
        ...
        Dim Carga_Comando_O1 As New OleDb.OleDbCommand("Select IQ_Design.*, IQ_PuntosAtencion.IQPUntos_Descripcion from IQ_Design join IQ_PuntosAtencion on IQ_Design.IIQDesign_Punto = IQ_PuntosAtencion.IQPuntos_Codigo where IQDesign_Oficina = '' and IQDesign_Area = '' and IQDEsign_Punto = '" & Alcance_Recfiles(2) & "' And IQDesign_Tipo = 'L' Order by IQDesign_Orden", Carga_Coneccion_O1)
...
Busca_2:
        ...
        Dim Carga_Coneccion_O2 As New OleDb.OleDbConnection(Cnn_Central_Server)
        Carga_Coneccion_O2.Open()
        Dim Carga_Comando_O2 As New OleDb.OleDbCommand("Select * from IQ_Design where IQDesign_Oficina = '' and IQDesign_Area = '" & Alcance_Recfiles(1) & "' and IQDEsign_Punto = '' And IQDesign_Tipo = 'L' Order by IQDesign_Orden", Carga_Coneccion_O2)
        Dim Carga_Reader_O2 As OleDb.OleDbDataReader = Carga_Comando_O2.ExecuteReader(CommandBehavior.CloseConnection)
        While Carga_Reader_O2.Read
            If IsDBNull(Carga_Reader_O2.GetValue(0)) = False Then
                encontrado = True
...
Busca_3:
        ...
        Dim Carga_Comando_O3 As New OleDb.OleDbCommand("Select IQ_Design.*, Iq_Oficinas.IQOficinas_Descripcion,  Iq_Oficinas.IQOficinas_Consolidacion from IQ_Design join IQ_Oficinas on IQDesign_Oficina = IQOficinas_Codigo where IQDesign_Oficina = '" & Alcance_Recfiles(0) & "' and IQDesign_Area = '' and IQDEsign_Punto = '' And IQDesign_Tipo = 'L' Order by IQDesign_Orden", Carga_Coneccion_O3)
